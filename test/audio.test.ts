...
describe("sfx", () => {
  ...
  it("exports all sound effect functions", async () => {
    ...
    expect(typeof mod.playClear).toBe("function");
    expect(typeof mod.playSignature).toBe("function");
  });

  it("playSignature accepts valid characters", async () => {
    const mod = await import("../src/audio/sfx");
    // Should not throw
    mod.playSignature("Rexy");
    mod.playSignature("Trikey");
    mod.playSignature("Sera");
  });
});
