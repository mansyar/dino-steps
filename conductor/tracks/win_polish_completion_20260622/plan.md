...
- [ ] Synthesize a universal cartoon chomping sound using existing AudioContext/synth infrastructure
- [ ] Ensure it respects the existing mute state (check `persisted.muted` like other SFX)

### Task: Integrate nom-nom into win sequence and remove win overlay
- [ ] In `main.ts`, call `playNomNom()` before `playSuccess()` in the win handler (GDD §8.2: nom-nom → success chime order)
- [ ] Remove `showWinOverlay()` / `hideWinOverlay()` DOM manipulation calls from win sequence
- [ ] Remove or hide the `winEl` DOM element and its CSS
- [ ] Verify auto-advance timer still fires after celebration duration (no regression)
